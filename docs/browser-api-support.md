...
| `document.cookie`, `document.domain` | `document.cookie` reads and writes the page-scoped C++ CookieJar shared with document/sub-resource/fetch/XHR loads. Basic `Domain`, `Path`, `Secure`, `HttpOnly`, `Expires`, and `Max-Age` handling is implemented; `HttpOnly` cookies are sent on matching requests but hidden from and protected against `document.cookie` overwrite/delete. `document.domain` exposes the current hostname. | `test_document_domain_and_cookie_jar`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr` |
...
| Static classic scripts | Standard JavaScript MIME types execute in document order. External scripts are fetched through `ResourceLoader::load_all` and still execute in document order. Script requests use the shared CookieJar and response `Set-Cookie` updates it. Non-standard script types stay inert. `async`/`defer` timing is not browser-complete. | `test_external_script_via_resource_loader`, `test_external_scripts_load_in_document_order`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr`, `test_non_javascript_script_types_are_not_executed` |
| Dynamic classic scripts | DOM-inserted classic scripts load through `ResourceLoader`, use the shared CookieJar, set `document.currentScript`, dispatch `load`/`error`, and do not rerun when moved. `LoadOptions::js_resource_load_policy` and JS resource budgets can reject JS-initiated dynamic script loads without affecting parser-discovered scripts. Dynamic module scripts are not implemented. | `test_dynamic_script_insertion_executes_classic_scripts`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr`, `test_js_resource_policy_block_all_keeps_parser_scripts`, `test_js_resource_budgets_limit_count_bytes_and_time` |
...
| `fetch`, `Request`, `Response` | Uses the host `ResourceLoader`, passes method/body/headers, returns status/statusText/body/final URL and response headers, hides `Set-Cookie`, honors `credentials: omit/same-origin/include` for Cookie/Set-Cookie handling, and participates in page readiness until its promise settles. `LoadOptions::js_resource_load_policy` and JS resource budgets can reject all, cross-origin, or over-budget JS-initiated loads. No real CORS model, streaming body, full redirect modes, or in-flight transport cancellation. | `test_request_response_fetch_object_shims`, `test_xhr_and_fetch_load_through_resource_loader`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr`, `test_fetch_xhr_credentials_control_cookie_injection`, `test_page_readiness_ready_waits_for_timer_fetch_and_dom_stable`, `test_js_resource_policy_same_origin_blocks_cross_origin_loads`, `test_js_resource_budgets_limit_count_bytes_and_time` |
| `XMLHttpRequest` | Uses `ResourceLoader`, supports method/body/headers, response text/json, ready-state transitions, status/statusText/responseURL, response header access with `Set-Cookie` hidden, `withCredentials` CookieJar behavior, abort/error/load events, and page-readiness participation while an async send is pending. `LoadOptions::js_resource_load_policy` and JS resource budgets can reject all, cross-origin, or over-budget JS-initiated loads. Progress events, timeout behavior, upload progress, streaming, and in-flight transport cancellation are limited. | `test_xhr_and_fetch_load_through_resource_loader`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr`, `test_fetch_xhr_credentials_control_cookie_injection`, `test_xhr_event_handler_exceptions_are_reported`, `test_js_resource_policy_block_all_keeps_parser_scripts`, `test_js_resource_budgets_limit_count_bytes_and_time` |
...
| CSSOM: `CSSRule`, `CSSStyleRule`, `CSSMediaRule`, `CSSStyleSheet`, `document.styleSheets` | Basic rule/declaration parsing, insertion, deletion, and stylesheet discovery. DOM-connected and render-pipeline stylesheet requests use the shared CookieJar, dispatch `load`/`error`, and participate in page readiness where JS-visible. `disabled`, `adoptedStyleSheets`, and shadow-tree styles are JS-visible but do not affect litehtml rendering or `getComputedStyle`. | `test_cssom_stylesheets_rules_declarations_and_cascade`, `test_cssom_dynamic_sheets_media_disabled_and_adopted`, `test_page_readiness_image_and_stylesheet_load_events`, `test_shared_cookie_jar_render_resources` |
...
| `Image` / `HTMLImageElement` | `new Image()` creates an `img`; `src` schedules a ResourceLoader image request through the shared CookieJar, sets `complete`, dispatches `load`/`error`, and participates in page readiness. Width/height and `decode()` are minimal; `naturalWidth`/`naturalHeight` and real decode-state semantics are not implemented. Rendering image decode is handled by the C++ image pipeline and render image requests also use the shared CookieJar. | `test_page_readiness_image_and_stylesheet_load_events`, `test_shared_cookie_jar_render_resources`; rendering image tests cover the C++ pipeline. |
