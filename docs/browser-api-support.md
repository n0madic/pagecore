...
| Dynamic classic scripts | DOM-inserted classic scripts load through `ResourceLoader`, set `document.currentScript`, dispatch `load`/`error`, and do not rerun when moved. `LoadOptions::js_resource_load_policy` and JS resource budgets can reject JS-initiated dynamic script loads without affecting parser-discovered scripts. Dynamic module scripts are not implemented. | `test_dynamic_script_insertion_executes_classic_scripts`, `test_js_resource_policy_block_all_keeps_parser_scripts`, `test_js_resource_budgets_limit_count_bytes_and_time` |
...
| `fetch`, `Request`, `Response` | Uses the host `ResourceLoader`, passes method/body/headers, returns status/body/basic headers, and participates in page readiness until its promise settles. `LoadOptions::js_resource_load_policy` and JS resource budgets can reject all, cross-origin, or over-budget JS-initiated loads. No real CORS model, streaming body, redirect modes, credential enforcement, or fetch `AbortSignal` cancellation. | `test_request_response_fetch_object_shims`, `test_xhr_and_fetch_load_through_resource_loader`, `test_page_readiness_ready_waits_for_timer_fetch_and_dom_stable`, `test_js_resource_policy_same_origin_blocks_cross_origin_loads`, `test_js_resource_budgets_limit_count_bytes_and_time` |
| `XMLHttpRequest` | Uses `ResourceLoader`, supports method/body/headers, response text/json, ready-state transitions, abort/error/load events, and participates in page readiness while an async send is pending. `LoadOptions::js_resource_load_policy` and JS resource budgets can reject all, cross-origin, or over-budget JS-initiated loads. Progress events, timeout behavior, upload progress, streaming, and full response-header fidelity are limited. | `test_xhr_and_fetch_load_through_resource_loader`, `test_xhr_event_handler_exceptions_are_reported`, `test_js_resource_policy_block_all_keeps_parser_scripts`, `test_js_resource_budgets_limit_count_bytes_and_time` |
...
| CSSOM: `CSSRule`, `CSSStyleRule`, `CSSMediaRule`, `CSSStyleSheet`, `document.styleSheets` | Basic rule/declaration parsing, insertion, deletion, and stylesheet discovery. DOM-connected `link rel=stylesheet` requests dispatch `load`/`error` and participate in page readiness. `disabled`, `adoptedStyleSheets`, and shadow-tree styles are JS-visible but do not affect litehtml rendering or `getComputedStyle`. | `test_cssom_stylesheets_rules_declarations_and_cascade`, `test_cssom_dynamic_sheets_media_disabled_and_adopted`, `test_page_readiness_image_and_stylesheet_load_events` |
...
| `Image` / `HTMLImageElement` | `new Image()` creates an `img`; `src` schedules a ResourceLoader image request, sets `complete`, dispatches `load`/`error`, and participates in page readiness. Width/height and `decode()` are minimal; `naturalWidth`/`naturalHeight` and real decode-state semantics are not implemented. Rendering image decode is handled by the C++ image pipeline. | `test_page_readiness_image_and_stylesheet_load_events`; rendering image tests cover the C++ pipeline. |
