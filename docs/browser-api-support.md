...
| Static classic scripts | Standard JavaScript MIME types execute in document order. External scripts are fetched through `ResourceLoader::load_all` and still execute in document order. Script requests use the shared CookieJar, carry the document URL as referrer, and response `Set-Cookie` updates the jar. Non-standard script types stay inert. `async`/`defer` timing is not browser-complete. | `test_external_script_via_resource_loader`, `test_external_scripts_load_in_document_order`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr`, `test_non_javascript_script_types_are_not_executed` |
| Dynamic classic scripts | DOM-inserted classic scripts load through `ResourceLoader`, use the shared CookieJar, carry the document URL as referrer by default, set `document.currentScript`, dispatch `load`/`error`, and do not rerun when moved. `LoadOptions::js_resource_load_policy` and JS resource budgets can reject JS-initiated dynamic script loads without affecting parser-discovered scripts. Dynamic module scripts are not implemented. | `test_dynamic_script_insertion_executes_classic_scripts`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr`, `test_js_resource_policy_block_all_keeps_parser_scripts`, `test_js_resource_budgets_limit_count_bytes_and_time` |
