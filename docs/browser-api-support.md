...
| `MutationObserver` | Child-list, attribute, and character-data records, `attributeFilter`, old-value handling, and delivery participation in page readiness. | `test_dom_fragment_range_serializer_and_mutation_observer`, `test_text_content_mutation_observer_records_nodes`, `test_mutation_observer_old_value`, `test_page_readiness_ready_waits_for_timer_fetch_and_dom_stable` |
...
| Basic timers | `setTimeout`, `clearTimeout`, `setInterval`, `clearInterval`, deterministic idle draining, readiness-aware one-shot timer snapshots, and logged callback errors. A zero wait budget leaves timer callbacks pending while still draining microtasks. Intervals do not permanently block readiness. | `test_timers_and_events`, `test_timer_wait_budget`, `test_zero_wait_does_not_run_timer_callbacks`, `test_page_readiness_wait_until_load_skips_timers`, `pagecore_dom_shim_unit_tests` |
...
| Dynamic classic scripts | DOM-inserted classic scripts load through `ResourceLoader`, use the shared CookieJar, carry the document URL as referrer by default, set `document.currentScript`, dispatch `load`/`error`, and do not rerun when moved. DOM-created classic scripts expose `async === true` by default; explicit `script.async = false` uses an ordered FIFO queue. `LoadOptions::js_resource_load_policy` and JS resource budgets can reject JS-initiated dynamic script loads without affecting parser-discovered scripts. Dynamic module scripts are not fetched or executed. | `test_dynamic_script_insertion_executes_classic_scripts`, `test_dynamic_module_scripts_are_not_executed`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr`, `test_js_resource_policy_block_all_keeps_parser_scripts`, `test_js_resource_budgets_limit_count_bytes_and_time` |
...
| `fetch`, `Request`, `Response` | Uses the host `ResourceLoader`, passes method/body/headers/referrer, returns status/statusText/body/final URL and response headers, hides `Set-Cookie`, honors `credentials: omit/same-origin/include` for Cookie/Set-Cookie handling, and participates in page readiness until its promise settles. `Request.referrer` supports `about:client`, URL values, empty string, and `no-referrer`; HTTP transport sanitizes emitted `Referer` to HTTP(S), strips userinfo/fragments, and suppresses non-HTTP(S) referrers. Full referrer-policy processing is not implemented. `LoadOptions::js_resource_load_policy` and JS resource budgets can reject all, cross-origin, or over-budget JS-initiated loads. No real CORS model, streaming body, full redirect modes, or in-flight transport cancellation. | `test_request_response_fetch_object_shims`, `test_xhr_and_fetch_load_through_resource_loader`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr`, `test_fetch_xhr_credentials_control_cookie_injection`, `test_cookie_attributes_path_domain_expires_secure_samesite`, `test_curl_loader_sends_user_agent_and_sanitized_referer_on_network_paths`, `test_page_readiness_ready_waits_for_timer_fetch_and_dom_stable`, `test_js_resource_policy_same_origin_blocks_cross_origin_loads`, `test_js_resource_budgets_limit_count_bytes_and_time` |
| `XMLHttpRequest` | Uses `ResourceLoader`, supports method/body/headers, sends the document URL as referrer by default after the same HTTP transport sanitization, exposes response text/json, ready-state transitions, status/statusText/responseURL, response header access with `Set-Cookie` hidden, `withCredentials` CookieJar behavior, abort/error/load events, and page-readiness participation while an async send is pending. `LoadOptions::js_resource_load_policy` and JS resource budgets can reject all, cross-origin, or over-budget JS-initiated loads. Progress events, timeout behavior, upload progress, streaming, and in-flight transport cancellation are limited. | `test_xhr_and_fetch_load_through_resource_loader`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr`, `test_fetch_xhr_credentials_control_cookie_injection`, `test_curl_loader_sends_user_agent_and_sanitized_referer_on_network_paths`, `test_xhr_event_handler_exceptions_are_reported`, `test_js_resource_policy_block_all_keeps_parser_scripts`, `test_js_resource_budgets_limit_count_bytes_and_time` |
