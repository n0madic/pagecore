...
| `document.cookie`, `document.domain` | `document.cookie` reads and writes the page-scoped C++ CookieJar shared with document/sub-resource/fetch/XHR loads. Basic `Domain`, `Path`, `Secure`, `HttpOnly`, `Expires`, `Max-Age`, and `SameSite` handling is implemented; `SameSite=None` requires `Secure`, unspecified cookies behave as default-Lax for cross-site subresource requests, and same-site checks use a simple schemeful registrable-domain heuristic without a Public Suffix List. `HttpOnly` cookies are sent on matching requests but hidden from and protected against `document.cookie` overwrite/delete. `document.domain` exposes the current hostname. | `test_document_domain_and_cookie_jar`, `test_shared_cookie_jar_document_scripts_fetch_and_xhr`, `test_cookie_attributes_path_domain_expires_secure_samesite` |
