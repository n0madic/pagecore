...
- `Page::display_list(RenderOptions)` serializes the JS-mutated Lexbor DOM, runs the configured layout engine, and returns backend-neutral commands. The result is memoized: repeated `display_list`/`render` calls reuse the cached list while the document layout mutation version, viewport, scale, base URL, external-resource flag, render resource mode, and render-only layout serialization mode are unchanged (the cache is invalidated by layout-affecting DOM mutations, `load_html`, and `set_resource_loader`/`set_layout_engine_factory`, but not by service-only DOM mutations such as `data-*`/`aria-*` attributes unless current CSS contains matching attribute selectors). For final rendering only, PageCore may transiently materialize cached JS-measured widths onto JS-positioned absolute elements so litehtml paints the same used width the page script saw; this does not mutate `serialize_html()` output and is not used for `getComputedStyle()` or JS geometry reads. The materialized render width is stored separately from the latest JS geometry cache, so later exact/snapshot geometry after `position:absolute` cannot overwrite the earlier browser-facing measured width. When no JS measurement exists, a first render pass can snapshot corrected `position:absolute; width:<percent>; box-sizing:border-box` widths and rebuild with transient width overrides, covering common Masonry/Isotope-style grids. Render-only width materialization is guarded by the cached element's ancestor inline-style fingerprint and layout-mutation evidence: complete history is checked directly, and when the capped history rolls over PageCore consults per-node blocker watermarks so unrelated DOM churn can still preserve the measured width while self/ancestor mutations that could affect sizing fail closed. `Page::layout(RenderOptions)` still returns a freshly built engine for callers that need the engine itself.
...
`layout_mutation_reason`, and `property`. A
`styled_document_cache_reason` prefixed with `bounded_mode:` means a heavy-page
single-property computed-style or cached geometry read used the bounded fallback
instead of forcing another styled-document rebuild. A prefix of
`preflight_append_child:` is only used by the single-property computed-style
path when it can answer from inline style, cache, or a conservative default
before paying the first rebuild for a large, newly appended subtree.
