...
- `ResourceLoader::load_all` loads a batch of resources in request order, throwing the first request-order error like `load`. The default implementation is serial; `CurlResourceLoader` overrides it to fetch concurrently with a single-threaded `curl_multi` driver (bounded to browser-like 6 connections per host), and `CachingResourceLoader` serves hits then batches only the misses. `Page` scans the whole document for external `<script src>` and prefetches them through `load_all`, while still executing them in document order.
