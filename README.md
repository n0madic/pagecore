...
- Before each render, `Page` scans the document for `<img src>` and `<link rel=stylesheet href>`, resolves them the way the litehtml container will (honouring a `<base href>`), and warms a per-render `CachingResourceLoader` with a single concurrent `Lenient` `load_all`. The layout's otherwise-synchronous, one-at-a-time stylesheet and image requests then hit the warm cache instead of blocking on a network round-trip each, turning the resource phase from O(N·latency) into O(latency) (bounded by the per-host connection cap). A failed prefetch never aborts the render — litehtml falls back to its placeholder.
